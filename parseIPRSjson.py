...
#'entry' : {'accession' : 'IPR001356','name' : 'Homeobox_dom','description' : 'Homeobox domain','type' : 'DOMAIN','goXRefs' : [ {'name' : 'DNA binding','databaseName' : 'GO','category' : 'MOLECULAR_FUNCTION','id' : 'GO:0003677'}]
    
    #pprint(match)
    sig_ent = match['signature']['entry']
    if sig_ent:
      sig_ent_acc = match['signature']['entry']['accession']
      if sig_ent_acc.startswith('IPR'):
        interpro_acc=sig_ent_acc
        if match['signature']['entry']['goXRefs']:
          for goXRef in match['signature']['entry']['goXRefs']:
            name=goXRef['name']
            cat=goXRef['category']
            Go_id=goXRef['id']
            GO_terms=f"{Go_id} {name} {cat}"
            print(f"InterPro2Go terms:{GO_terms}")
        
          if GO_terms not in InterPRO2GO:
            InterPRO2GO[GO_terms]=0
          InterPRO2GO[GO_terms]+=1
...
print("==== IPR GO TERM Counts ===")
for info,count in sorted(InterPRO2GO.items(), key=lambda x:x[1], reverse=True):
  if count > 1:
    print(info,count)
